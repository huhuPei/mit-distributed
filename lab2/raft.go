...
type Raft struct {
	...
	// Volatile state, empty if never become leader
...
func (rf *Raft) persist() {
	...
	//	binary.Write(wbuf, binary.SmallEndian, int32(itemBuf.Len()))
	if enc.Encode(rf.currentTerm) != nil ||
		enc.Encode(rf.votedFor) != nil {
		Debug(dError, "S%d Failed to save state T:%d VF:%d", rf.me, rf.currentTerm, rf.votedFor)
	} else {
		Debug(dPersist, "S%d Saved State T:%d VF:%d", rf.me, rf.currentTerm, rf.votedFor)
	}
	if enc.Encode(rf.log) != nil {
		Debug(dError, "S%d Failed to save log T:%d VF:%d", rf.me, rf.currentTerm, rf.votedFor)
	} else {
		Debug(dLog2, "S%d Saved Log (-1, 0) %v", rf.me, rf.log)
	}
	data := wbuf.Bytes()
	rf.persister.SaveRaftState(data)
}
...
func (rf *Raft) readPersist(data []byte) {
	...
	if dec.Decode(&currentTerm) != nil ||
		...
		Debug(dPersist, "S%d Restored State T:%d VF:%d", rf.me, rf.currentTerm, rf.votedFor)
	}
	if dec.Decode(&log) != nil {
		Debug(dError, "S%d Failed to restore log.")
	} else {
		rf.log = log
		Debug(dLog2, "S%d Restored log (-1, 0) %v", rf.me, rf.log)
	}
}
...
type AppendEntriesReply struct {
	...
	Len     int  // len of the follower's log
}
...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if args.Term < rf.currentTerm {
		...
		if args.PrevLogIndex > len(rf.log)-1 {
			*reply = AppendEntriesReply{Success: false, XTerm: -1, XIndex: -1, Len: len(rf.log)}
			...
			for offset := 1; offset < len(args.Entries)+1; offset++ {
				...
				if index > len(rf.log)-1 || rf.log[index].Term != args.Entries[offset-1].Term {
					rf.log = rf.log[:index]
					Debug(dLog, "S%d Remove Log, conflict index:%d from %v", rf.me, index, rf.log)
					rf.log = append(rf.log, args.Entries[offset-1:]...)
					Debug(dLog, "S%d Replicate Log, start from index:%d result %v", rf.me, index, rf.log)
					rf.persist()
					break
				}
			}
...
func (rf *Raft) Start(command interface{}) (int, int, bool) {
	...
	if rf.state == Leader && rf.killed() == false {
		...
		rf.persist()
		Debug(dClient, "S%d Append one new log entry - %v, start agreement.", rf.me, rf.log[index])
			...
			if reply.Success {
				...
				if reply.XTerm == -1 {
					...
					for {
						...
						if rf.log[rf.nextIndex[peer]].Term == reply.XTerm {
							break
						}
						if rf.log[rf.nextIndex[peer]].Term < reply.XTerm {
							rf.nextIndex[peer] = reply.XIndex
...
func (rf *Raft) sendAppendBroadcastEL(isHeartbeat bool) {
	...
	for peer := range rf.peers {
		if peer != rf.me {
			if isHeartbeat || len(rf.log)-1 > rf.nextIndex[peer] {
...
func (rf *Raft) startRequestVote(peer int, votes *int, args *RequestVoteArgs) {
	...
	if ok {
		...
		runHeartbeat := false
		if rf.currentTerm == args.Term {
			if reply.VoteGranted && *votes <= len(rf.peers)/2 {
...
func (rf *Raft) applier() {
	for rf.killed() == false {
		...
		if rf.lastApplied < rf.commitIndex {
			rf.lastApplied += 1
			committedLog := rf.log[rf.lastApplied]
			applyMsg := ApplyMsg{}
